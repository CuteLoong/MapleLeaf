...
namespace MapleLeaf {
SSRSubrender::SSRSubrender(const Pipeline::Stage& pipelineStage, SSRData ssrData)
    : Subrender(pipelineStage)
    , ssrData(ssrData)
    , pipelineCompute("Shader/SSR/SSR.comp", {SSRSubrender::GetDefines()}, false)
{}

void SSRSubrender::PreRender(const CommandBuffer& commandBuffer) {}

void SSRSubrender::Render(const CommandBuffer& commandBuffer) {}

void SSRSubrender::PostRender(const CommandBuffer& commandBuffer)
{
    ...
    uniformSSRData.Push("hiZMaxLevel", static_cast<float>(minHiZ->GetMipLevels()));
...
std::vector<Shader::Define> SSRSubrender::GetDefines()
{
