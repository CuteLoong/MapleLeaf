...
namespace MapleLeaf {
HBAOStereoWithOcclusionInfoSubrender::HBAOStereoWithOcclusionInfoSubrender(const Pipeline::Stage& pipelineStage, HBAOData hbaoData)
    : Subrender(pipelineStage)
    , hbaoData(hbaoData)
    , pipeline(PipelineGraphics(pipelineStage, {"Shader/AO/HBAOStereoWithOcclusionInfo.vert", "Shader/AO/HBAOStereoWithOcclusionInfo.frag"}, {}, {},
...
void HBAOStereoWithOcclusionInfoSubrender::PreRender(const CommandBuffer& commandBuffer) {}

void HBAOStereoWithOcclusionInfoSubrender::Render(const CommandBuffer& commandBuffer)
{
    auto camera = Scenes::Get()->GetScene()->GetCamera();
    camera->PushUniforms(uniformCamera);

    float pixelRadius = camera->GetStereoPixelHeight() * hbaoData.sampleRadius / (2.0f * glm::tan(camera->GetFieldOfView() / 2.0f));
    ...
    descriptorSet.Push("thicknessMap", Graphics::Get()->GetNonRTAttachment("ThicknessMap"));
...
void HBAOStereoWithOcclusionInfoSubrender::PostRender(const CommandBuffer& commandBuffer) {}

std::shared_ptr<Image2d> HBAOStereoWithOcclusionInfoSubrender::ComputeNoise(uint32_t width, uint32_t height, uint32_t numRays)
{
