    ...
    : instance(instance)
{
    ...
    if (!physicalDevice) throw std::runtime_error("Vulkan runtime error, failed to find a suitable GPU");
...
VkPhysicalDevice PhysicalDevice::ChoosePhysicalDevice(const std::vector<VkPhysicalDevice>& devices)
{
    ...
    for (const auto& device : devices) where = rankedDevices.insert(where, {ScorePhysicalDevice(device), device});
    if (rankedDevices.rbegin()->first > 0) return rankedDevices.rbegin()->second;
                                     ...
                                     const std::vector<VkExtensionProperties>& extensionProperties)
{
    ...
    case 1: ss << "Integrated"; break;
    case 2: ss << "Discrete"; break;
    case 3: ss << "Virtual"; break;
    case 4: ss << "CPU"; break;
    default: ss << "Other " << physicalDeviceProperties.deviceType;
    }
    ...
    default: ss << " \"" << physicalDeviceProperties.vendorID << '\"';
    }

    ss << " " << std::quoted(physicalDeviceProperties.deviceName) << '\n';

    uint32_t supportedVersion[3] = {VK_VERSION_MAJOR(physicalDeviceProperties.apiVersion),
                                    VK_VERSION_MINOR(physicalDeviceProperties.apiVersion),
                                    VK_VERSION_PATCH(physicalDeviceProperties.apiVersion)};
    ss << "API Version: " << supportedVersion[0] << "." << supportedVersion[1] << "." << supportedVersion[2] << '\n';

    ss << "Extensions: ";
    for (const auto& extension : extensionProperties) ss << extension.extensionName << ", ";
