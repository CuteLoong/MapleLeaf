...
public:
    ...
    T* Get() const
    {
        ...
        if (auto it = subrenders.find(typeId); it != subrenders.end() && it->second) return static_cast<T*>(it->second.get());
...
private:
    // first is render stage, second is the index of subrender
